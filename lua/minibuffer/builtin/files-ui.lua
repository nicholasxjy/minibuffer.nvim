...
local M = {}
local colors = {
  Staged = { "#10B981", 2 }, Modified = { "#F59E0B", 3 },
  Deleted = { "#EF4444", 1 }, Renamed = { "#8B5CF6", 5 },
  Untracked = { "#10B981", 2 }, Ignored = { "#4B5563", 8 },
}
local signs = {
...
local function category(status)
  if status == "unknown" then return "Untracked" end
...
function M.shorten(path, width)
  local function size(text) return vim.fn.strdisplaywidth(text) end
  local function truncate(text, limit)
    local result = ""
    for _, char in ipairs(vim.fn.split(text, "\\zs")) do
      if size(result .. char) > limit then break end
      result = result .. char
    end
    return result
  end
  if width <= 0 then return "" end
  if size(path) <= width then return path end
  local parts = vim.split(path, "/", { plain = true })
  local left, right = 1, #parts
  local function middle(a, b)
    local hidden = b - a - 1
    local marker = hidden > 3 and ("." .. hidden .. ".") or string.rep(".", hidden)
    return table.concat(parts, "/", 1, a) .. "/" .. marker .. "/" .. table.concat(parts, "/", b)
  end
  if #parts > 2 and size(middle(left, right)) <= width then
    while right > left + 2 do
      local changed = false
      if size(middle(left, right - 1)) <= width then right, changed = right - 1, true end
      if right > left + 2 and size(middle(left + 1, right)) <= width then left, changed = left + 1, true end
  ...
  if #parts > 2 then
    ...
    if size(marker .. "/" .. tail) <= width then
      ...
      return (room >= 0 and (truncate(parts[1], room) .. "/") or "") .. marker .. "/" .. tail
    end
...
function M.setup()
  ...
  for name, color in pairs(colors) do
    for _, prefix in ipairs({ "FFFGit", "FFFGitSign" }) do
      vim.api.nvim_set_hl(0, prefix .. name, { fg = color[1], ctermfg = color[2], default = true })
    end
    vim.api.nvim_set_hl(0, "FFFGitSign" .. name .. "Selected", {
      fg = color[1], ctermfg = color[2], bg = visual.bg, ctermbg = visual.ctermbg, default = true,
    })
  end
  ...
  if not selected.fg then selected = vim.api.nvim_get_hl(0, { name = "Number", link = false }) end
  vim.api.nvim_set_hl(0, "FFFSelectedActive", {
    ...
    bg = visual.bg, ctermbg = visual.ctermbg, default = true,
  })
  vim.api.nvim_create_autocmd("ColorScheme", {
    group = vim.api.nvim_create_augroup("MinibufferFilesHighlights", { clear = true }), callback = M.setup,
  })
end

function M.icon(item)
  local ok, icons = pcall(require, "nvim-web-devicons")
  if ok then return icons.get_icon(item.name, nil, { default = true }) end
...
function M.format(item, ctx, opts, current_file)
  ...
  local chunks = {}
  if item.icon then
    chunks[#chunks + 1] = { text = item.icon .. " ", hl = current and "Comment" or item.icon_hl }
  end
  local name_hl
  ...
  local function append(text, offset, hl, matchable)
    for i, char in ipairs(vim.fn.split(text, "\\zs")) do
      chunks[#chunks + 1] = { text = char, hl = matchable ~= false and item.matches and item.matches[offset + i - 1] and opts.hl.matched or hl }
    end
  end
  ...
  if not opts.fuzzy_query_highlighting and ctx.input ~= "" then
    local line = ""
    for _, chunk in ipairs(chunks) do line = line .. chunk.text end
    local first, last = line:find(ctx.input, 1, true)
    if first then
      local offset = 0
      for _, chunk in ipairs(chunks) do
        if offset >= first - 1 and offset < last then chunk.hl = opts.hl.matched end
...
function M.decorate(sess, opts, current_file)
  ...
  for i = sess._scroll_offset + 1, math.min(#sess._items, sess._scroll_offset + height) do
    ...
    if opts.show_git_status == false then char = nil end
    ...
    if char then
      vim.api.nvim_buf_set_extmark(buf, ns, row, 0, { sign_text = char, sign_hl_group = group, priority = 1001 })
    end
    if item.path == current_file and opts.current_file_label ~= "" then
      vim.api.nvim_buf_set_extmark(buf, ns, row, 0, {
        virt_text = { { " " .. opts.current_file_label, active and opts.hl.cursor or "Comment" } },
