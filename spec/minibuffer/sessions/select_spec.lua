...
describe("minibuffer.sessions.select", function()
  local SelectSession
  local cmd_buf
  local cmd_win
  local entry_buf
  ...
  before_each(function()
    package.loaded["minibuffer.sessions.select"] = nil
    SelectSession = require("minibuffer.sessions.select")
  ...
  local function new_session(opts)
    ...
    opts.filter_fn = opts.filter_fn or function(ctx)
      return ctx.items
    end
  ...
  describe(".new", function()
    it("creates a closed session with defaults", function()
      ...
      assert.equal("Select: ", session.prompt)
      assert.equal(15, session.max_height)
      assert.is_false(session.multi)
      ...
      assert.same({}, session._selected_indices)
    ...
    it("requires a filter function", function()
      assert.has_error(function()
        SelectSession.new({
    ...
    it("provides a single-select footer by default", function()
      local session = new_session()

      local footer = session.footer_fn(session:get_ctx())
    ...
    it("provides a multi-select footer by default", function()
      local session = new_session({ multi = true })
  ...
  describe(":type", function()
    it("returns select", function()
      assert.equal("select", new_session():type())
    end)
  end)
  ...
  describe(":pre_start", function()
    ...
    it("errors when the entry buffer cannot be created", function()
      local calls = 0
      helper.stub_method(vim_api, "nvim_create_buf", function()
        calls = calls + 1
        if calls == 1 then
      ...
      assert.has_error(function()
        new_session():pre_start()
      end, "Failed to create _entry minibuffer")
    end)
  end)
  ...
  describe(":render", function()
    ...
    it("updates the footer", function()
      session._items = { "one", "two" }

      local footer_fn = spy.new(function(ctx)
        assert.same({ "one", "two" }, ctx.items)
        return { { "custom", "Comment" } }
      end)
      session.footer_fn = footer_fn

      session:render()

      assert.spy(footer_fn).called_at_least(1)
      assert.stub(vim_api.nvim_win_set_config).called_with(display_win, {
        footer = {
          { "custom", "Comment" },
        },
      })
    ...
    it("marks multi-selected items", function()
      session.multi = true
      session._items = { "one", "two", "three" }
      session._current_index = 2
      session._selected_indices = { 1, 2, 3 }

      session:render()

      assert
        .stub(vim_api.nvim_buf_set_extmark)
        .called_with(display_buf, state.ns, 0, 0, { line_hl_group = "MinibufferMultiSelected" })

      assert
        .stub(vim_api.nvim_buf_set_extmark)
        .called_with(display_buf, state.ns, 2, 0, { line_hl_group = "MinibufferMultiSelected" })
    end)

    it("does not multi-highlight the current item", function()
      session.multi = true
      session._items = { "one", "two" }
      session._current_index = 1
      session._selected_indices = { 1 }

      session:render()

      assert
        .stub(vim_api.nvim_buf_set_extmark)
        .called_with(display_buf, state.ns, 0, 0, { line_hl_group = "MinibufferSelection" })

      local found_multi = false
      for _, call in ipairs(vim_api.nvim_buf_set_extmark.calls) do
        if call.refs[5] and call.refs[5].line_hl_group == "MinibufferMultiSelected" then
          found_multi = true
        end
      end
      assert.is_false(found_multi)
    end)

    it("does not highlight selected items outside the viewport", function()
      session.multi = true
      session._items = { "1", "2", "3", "4", "5" }
      session._current_index = 3
      session._selected_indices = { 1, 5 }
      session.max_height = 3
      session._scroll_offset = 1
      ...
      assert.equal(0, multi_count)
    end)

    it("calls on_change with the current item", function()
      session._items = { "one", "two" }
      session._input = "foo"
      session._current_index = 2
  ...
  describe(":post_start", function()
    ...
    it("creates all default single-select mappings", function()
      session:post_start()
      ...
      assert.is_nil(lhs["<C-x>"])
      assert.is_nil(lhs["<C-a>"])
    end)

    it("creates multi-select mappings", function()
      session.multi = true
      ...
      assert.is_true(lhs["<C-x>"])
      assert.is_true(lhs["<C-a>"])
    end)
    ...
    it("refreshes results when there are no items", function()
      session.refresh_results = spy.new(function() end)

      session:post_start()

      assert.spy(session.refresh_results).called_at_least(1)
    end)

    it("does not refresh results when items already exist", function()
      session._items = { "one" }
      session.refresh_results = spy.new(function() end)

      session:post_start()

      assert.spy(session.refresh_results).called_at_most(0)
    end)
  end)

  describe(":refresh_results", function()
    it("does nothing when closed", function()
      local session = new_session()
    ...
    it("marks the session as loading and calls fetch_fn", function()
      ...
      local filter = spy.new(function(ctx)
        assert.equal("abc", ctx.input)
        assert.same({ "one", "two" }, ctx.items)
        assert.is_true(ctx.loading)
        return { "two" }
      end)

      session.fetch_fn = fetch
      session.filter_fn = filter
      session.render = spy.new(function() end)

      session:refresh_results()

      assert.spy(fetch).called_with("abc", match.is_function())
      assert.spy(filter).called_at_least(1)
      assert.is_false(session._loading)
      assert.same({ "two" }, session._items)
      assert.equal(1, session._current_index)
    ...
    it("resets selection when filtered results are empty", function()
      local session = new_session()
      session._closed = false
      session._current_index = 3
      session._scroll_offset = 4

      session.filter_fn = function()
        return {}
      end
    ...
    it("selects the first item when results are returned", function()
      local session = new_session()
      session._closed = false
      session._current_index = 3
      session._scroll_offset = 5
    ...
    it("clears multi selections when results are refreshed", function()
      local session = new_session({ multi = true })
      session._closed = false
      session._selected_indices = { 1, 2 }
    ...
    it("does not clear selections for single-select sessions", function()
      local session = new_session()
      session._closed = false
      session._selected_indices = { 1, 2 }
      ...
      assert.same({ 1, 2 }, session._selected_indices)
    end)
  ...
  describe(":accept", function()
    ...
    it("accepts the current item in single-select mode", function()
      local session = new_session()
      session._closed = false
      session._items = { "one", "two" }
      session._current_index = 2
      ...
      local on_accept = spy.new(function() end)
      session.on_accept = on_accept

      session:accept()

      assert.is_true(session._closed)
      assert.spy(on_accept).called_with({
        { item = "two", index = 1 },
      })
    end)

    it("returns the selected indices in multi-select mode", function()
      local session = new_session({ multi = true })
      session._closed = false
      session._items = { "one", "two", "three" }
      session._current_index = 2
      session._selected_indices = { 1, 3 }
      session._entry = { buf = entry_buf, win = entry_win }
      session._display = { buf = display_buf, win = display_win }
    ...
    it("accepts the current item when multi-select has no explicit selections", function()
      local session = new_session({ multi = true })
      session._closed = false
      session._items = { "one", "two" }
      session._current_index = 2
      session._selected_indices = {}
      session._entry = { buf = entry_buf, win = entry_win }
      session._display = { buf = display_buf, win = display_win }
    ...
    it("ignores invalid selected indices", function()
      local session = new_session({ multi = true })
      session._closed = false
      session._items = { "one", "two" }
      session._current_index = 1
      session._selected_indices = { 1, 99 }
      session._entry = { buf = entry_buf, win = entry_win }
      session._display = { buf = display_buf, win = display_win }
    ...
    it("does nothing when the current index is invalid", function()
      local session = new_session()
      session._closed = false
      session._items = { "one" }
      session._current_index = 0
      ...
      assert.is_false(session._closed)
      assert.spy(on_accept).called_at_most(0)
    end)

    it("does not fail when on_accept throws", function()
      local session = new_session()
      session._closed = false
      session._items = { "one" }
      session._current_index = 1
  ...
  describe(":is_selected", function()
    it("returns false for single-select sessions", function()
      local session = new_session()
      session._selected_indices = { 1 }

      assert.is_false(session:is_selected(1))
    end)

    it("returns true for a selected multi-select item", function()
      local session = new_session({ multi = true })
      session._selected_indices = { 1, 3 }

      assert.is_true(session:is_selected(1))
      assert.is_true(session:is_selected(3))
    end)

    it("returns false for an unselected multi-select item", function()
      local session = new_session({ multi = true })
      session._selected_indices = { 1, 3 }

      assert.is_false(session:is_selected(2))
    end)
  end)
  ...
  describe(":get_selected", function()
    it("returns the current item", function()
      ...
      assert.equal("two", session:get_selected())
    end)

    it("returns nil when there is no current selection", function()
      local session = new_session()
      session._items = {}
      session._current_index = 0

      assert.is_nil(session:get_selected())
    end)
  end)
  ...
  describe(":toggle_selection", function()
    ...
    before_each(function()
      session = new_session({ multi = true })
      session._closed = false
      session._items = { "one", "two", "three" }
      session._current_index = 2
      session.render = spy.new(function() end)
    ...
    it("does nothing in single-select mode", function()
      session.multi = false
    ...
    it("does nothing when there is no current selection", function()
      session._current_index = 0
    ...
    it("selects the current item", function()
      ...
      assert.same({ 2 }, session._selected_indices)
      assert.spy(session.render).called_at_least(1)
    end)
    ...
    it("deselects the current item", function()
      ...
      assert.same({ 1, 3 }, session._selected_indices)
      assert.spy(session.render).called_at_least(1)
    end)
  end)

  describe(":toggle_selection_all", function()
    local session
    ...
    it("selects all items when not all are selected", function()
      session._selected_indices = { 1 }

      session:toggle_selection_all()

      assert.same({ 1, 2, 3 }, session._selected_indices)
      assert.spy(session.render).called_at_least(1)
    end)

    it("deselects all items when all are selected", function()
      session._selected_indices = { 1, 2, 3 }
