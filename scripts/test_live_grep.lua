...
-- Separate picker invocations must not cancel each other's debounce or process.
local original_picker = picker
local first_fetch = picker.fetch_fn
grep({ cwd = root })
local results = {}
first_fetch("test1", function(value, err)
  assert(not err, err)
  results[1] = value
end)
picker.fetch_fn("nothing", function(value, err)
  assert(not err, err)
  results[2] = value
end)
assert(vim.wait(3000, function()
...
assert(#results[1] == 1 and results[1][1].line == 1)
assert(#results[2] == 1 and results[2][1].line == 2)
-- Closing cancels pending work; the same picker can fetch again when resumed.
local closed_callback = false
picker.fetch_fn("test1", function()
...
assert(not closed_callback, "closed picker must not deliver pending results")
local resumed
picker.fetch_fn("test2", function(value, err)
  assert(not err, err)
  resumed = value
end)
assert(vim.wait(3000, function()
...
-- Late process completion cannot overwrite a newer query or a closed picker.
local system, pending, killed = vim.system, {}, 0
...
local stale, latest = false, nil
picker.fetch_fn("old", function()
  stale = true
end)
assert(vim.wait(1000, function()
  return #pending == 1
end))
picker.fetch_fn("new", function(value)
  latest = value
end)
pending[1]({ code = 1, stdout = "" })
assert(not stale and killed == 1)
assert(vim.wait(1000, function()
  return #pending == 2
end))
picker.on_close()
pending[2]({ code = 1, stdout = "" })
assert(latest == nil and killed == 2)
vim.system = system
...
assert(
  vim.api.nvim_buf_get_lines(sess._entry.buf, 0, 1, false)[1] == "> test[12]",
  "initial query is inserted literally into the prompt buffer"
)
sess:refresh_results()
assert(vim.wait(3000, function()
  return not sess._loading
end))
...
for _, width in ipairs({ 24, 100 }) do
  ...
  for i = 1, #header - 1 do
    local text = ""
    for _, chunk in ipairs(header[i]) do
...
for i = 1, #many do
  ...
  local body =
    vim.api.nvim_buf_get_lines(sess._display.buf, 0, -1 - sess._header_height, false)
...
assert(
  vim.api.nvim_buf_get_name(vim.fn.getqflist()[1].bufnr) == path,
...
assert(
  ordered[1] == candidates[2] and ordered[2] == candidates[4],
  "invoking file group first"
)
assert(
  ordered[3] == candidates[1] and ordered[4] == candidates[3],
  "other groups retain their order"
)
grep({ cwd = root, current_file_first = false })
assert(picker.filter_fn({ items = candidates })[1] == candidates[1])
grep({ cwd = root, current_file_first = true })
assert(
  picker.filter_fn({ items = candidates })[1] == candidates[1],
  "unnamed buffer keeps default order"
)
vim.fn.delete(root, "rf")
print(
  "live grep layout, highlights, regex matches, navigation paths and empty results passed"
)
